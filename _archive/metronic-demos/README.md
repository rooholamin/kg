# KGHub demo routes (archived from navigation)

The main sidebar (`config/menu.config.jsx` → `MENU_SIDEBAR`) is scoped to the **Automation Magazine Dashboard** under `/dashboard/*`.

**KGHub’s stock demo pages remain in the App Router** so theme upgrades stay mergeable, but they are **not** linked from the product navigation after Milestone 1.

## Demo route groups (not in product menu)

- `/` — original KGHub “Light Sidebar” home (`app/(protected)/page.jsx`)
...
- `/user-management/*` — **still functional** KGHub user CRUD; linked from our `/dashboard/users` for admins who need the full module

## Policy

- **Do not delete** these routes unless a KGHub upgrade explicitly removes them.
