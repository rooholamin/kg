...
**Step 0, once per video (PHASE: execute only):** `generate_image` with `params.model: "nano_banana_2"` — ONE character anchor still: the real Reference Element ID value wrapped in `<<< >>>` (never the literal word "elementId" — see Golden Rule 1), a clean neutral front-facing composition in the Loft. No need to describe their appearance — the Reference Element carries that. This job id is reused as `start_image` for every avatar segment below — never regenerated mid-video (see Golden Rule 2).
