...
3. **One anchor still per video, chained into every character frame.** Generate exactly ONE character still (Step 0 of PHASE: stills). Each avatar (`hasCharacter: true`) segment then gets its OWN start frame, generated with that anchor passed as an image reference (`params.medias`, `role: "image"`) alongside the Reference Element ID — identity and wardrobe come from the anchor, pose and setting come from the prompt. That's what lets the character be somewhere other than the anchor's framing without drifting into a different-looking person. A character frame produced without the anchor chained in is an independent roll of the dice on her appearance. Never generate a second anchor mid-video unless a director note explicitly asks for a look change.
...
5. **Environment is TEXT, not an image ref.** Passing a location photo as an image ref causes "ref bleeding" — the model copies the reference photo's exact composition into your output instead of just matching its mood. Write the KG Media Loft descriptor straight into the `Scene:` portion of your prompt instead. The same reasoning applies to the video's recurring subject: keep it consistent by restating the plan's `subjectAnchor` text verbatim in every prompt that shows it, and only fall back to chaining an actual frame as an image reference where two shots must show literally the same object (a before/after pair), which the plan marks with `stillReferenceOrder`. Chain it everywhere and every shot ends up looking like the same photograph.
...
- **Step 0, once per video:** `generate_image` with `params.model: "nano_banana_2"` — ONE character anchor still: the real Reference Element ID value wrapped in `<<< >>>` (never the literal word "elementId" — Golden Rule 1), wearing the approved `characterLook`, a clean neutral front-facing composition in the Loft, at the configured `aspect_ratio` and `resolution` (Golden Rule 10). This is the canonical look every character frame derives from — never regenerated mid-video (Golden Rule 3).
- **Step 1, one frame per segment** — every segment, avatar and b-roll alike:
  - Avatar (`hasCharacter: true`): `nano_banana_2`, the real ID wrapped in `<<< >>>`, the anchor chained as an image reference, the segment's own `visualDescription`, and the approved `characterLook` restated.
  - B-roll (`hasCharacter: false`): that segment's actual subject, no character — plus the `subjectAnchor` text verbatim if the shot includes the video's recurring subject (Golden Rule 5).
...
`PHASE: regenerate_segment` redoes the `generate_video` step for one segment from its already-approved frame. Only generate a replacement frame if the complaint is about the frame rather than the motion — and if it's an avatar segment, chain the Step 0 anchor again exactly as in PHASE: stills.
