...
/** Platform display color for calendar events */
const SOCIAL_PLATFORM_COLOR = {
  instagram_carousel: 'amber',
  instagram_story: 'amber',
  linkedin: 'sky',
  twitter: 'slate',
};
...
/**
 * Map scheduled social posts to calendar event shapes.
 * @param {Array<{
 *   id: string;
 *   articleId: string;
 *   platform: string;
 ...
 * }>} posts
 */
export function socialPostsToCalendarEvents(posts) {
  if (!Array.isArray(posts) || posts.length === 0) return [];

  return posts
    .filter((p) => p.scheduledAt)
    .map((p) => {
      const date = parseISO(String(p.scheduledAt));
      const platformLabel = SOCIAL_PLATFORM_LABEL[p.platform] ?? p.platform;
      const title = p.article?.title
        ? `${platformLabel}: ${p.article.title}`
        : platformLabel;
      return {
        id: `social-${p.id}`,
        title,
        ...
        allDay: false,
        color: SOCIAL_PLATFORM_COLOR[p.platform] ?? 'amber',
        source: 'social',
